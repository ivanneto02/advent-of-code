...
def part2():
    ...
    return sum
```
Note: `ordered_up` uses `cmp_to_key` to mimic Python 2's old-style comparator functionality similar to C/C++. It allows us to take the two items being compared and create custom functionality for ordering the items. In this challenge, I used the `order_map` I made to make that decision.
